...
v_temp_ratio := p_temperature / NULLIF(v_machine.max_temperature::DOUBLE PRECISION, 0);
IF v_temp_ratio > 1.0 THEN -- Critical: Over max temperature
RETURN QUERY
SELECT 'critical'::TEXT,
    'thermal_runaway'::TEXT,
    (
        'CRITICAL: Temperature ' || ROUND(p_temperature::numeric, 1) || '°C exceeds maximum ' || v_machine.max_temperature || '°C'
    )::TEXT,
    p_temperature,
    v_machine.max_temperature::DOUBLE PRECISION,
    'emergency_stop'::TEXT,
    'red'::TEXT,
    CASE
        v_machine.type
        ...
        ELSE 'facility'
    END::TEXT,
    (p_temperature - v_machine.max_temperature::DOUBLE PRECISION * 0.8) / 5.0;
RETURN;
ELSIF v_temp_ratio > 0.95 THEN -- High: Near max
RETURN QUERY
SELECT 'high'::TEXT,
    'elevated_temperature'::TEXT,
    (
        'HIGH: Temperature ' || ROUND(p_temperature::numeric, 1) || '°C near maximum threshold'
    )::TEXT,
    p_temperature,
    v_machine.max_temperature::DOUBLE PRECISION * 0.95,
    ...
    3.0;
RETURN;
ELSIF v_temp_ratio > 0.85 THEN -- Medium: Warning level
RETURN QUERY
SELECT 'medium'::TEXT,
    'temperature_warning'::TEXT,
    (
        'WARNING: Elevated temperature ' || ROUND(p_temperature::numeric, 1) || '°C'
    )::TEXT,
    p_temperature,
    v_machine.max_temperature::DOUBLE PRECISION * 0.85,
...
v_vib_ratio := p_vibration / NULLIF(v_machine.max_vibration::DOUBLE PRECISION, 0);
IF v_vib_ratio > 1.0 THEN -- Critical vibration
RETURN QUERY
SELECT 'critical'::TEXT,
    'vibration_alert'::TEXT,
    (
        'CRITICAL: Vibration ' || ROUND(p_vibration::numeric, 4) || ' mm/s exceeds maximum ' || v_machine.max_vibration::DOUBLE PRECISION
    )::TEXT,
    p_vibration,
    v_machine.max_vibration::DOUBLE PRECISION,
    'schedule_maintenance'::TEXT,
    'red'::TEXT,
    'facility'::TEXT,
    4.0;
RETURN;
ELSIF v_vib_ratio > 0.85 THEN -- High vibration warning
RETURN QUERY
SELECT 'high'::TEXT,
    'increased_vibration'::TEXT,
    (
        'HIGH: Vibration ' || ROUND(p_vibration::numeric, 4) || ' mm/s above normal'
    )::TEXT,
    p_vibration,
...
CREATE OR REPLACE FUNCTION auto_create_incident_from_reading() RETURNS TRIGGER AS $$
DECLARE v_analysis RECORD;
v_machine_name TEXT;
v_exists BOOLEAN;
BEGIN -- Skip if this is not a real anomaly (is_anomaly flag or threshold breach)
IF NOT NEW.is_anomaly THEN -- Check if readings breach thresholds anyway
SELECT * INTO v_analysis
FROM analyze_sensor_thresholds(
        NEW.machine_id,
        NEW.temperature,
        NEW.vibration
    );
...
END IF;
ELSE -- It's marked as anomaly, create incident
SELECT * INTO v_analysis
FROM analyze_sensor_thresholds(
        NEW.machine_id,
        NEW.temperature,
...
IF NOT FOUND THEN v_analysis.severity := 'medium';
...
IF NOT v_exists THEN -- Create the incident
...
AFTER
INSERT ON sensor_readings FOR EACH ROW
    WHEN (
        NEW.is_anomaly = true
        OR NEW.temperature > 70
        OR NEW.vibration > 0.02
...
-- =====================================================
CREATE OR REPLACE FUNCTION backfill_incidents_from_readings(p_hours_back INT DEFAULT 24) RETURNS INT AS $$
DECLARE v_count INT := 0;
v_reading RECORD;
v_analysis RECORD;
v_machine_name TEXT;
v_exists BOOLEAN;
BEGIN FOR v_reading IN
SELECT sr.*,
...
ORDER BY sr.recorded_at DESC LOOP -- Check for existing incident
    v_exists := check_existing_incident(
        v_reading.machine_name,
        v_reading.recorded_at,
        300
    );
IF NOT v_exists THEN -- Get analysis
SELECT * INTO v_analysis
FROM analyze_sensor_thresholds(
...
SELECT 'Trigger active' as status,
    (
        SELECT COUNT(*)
        FROM aegis_incidents
